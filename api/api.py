    ...
    def get(self):
        ...
        * number of unusual spikes of sensors data
        ...
        if ( output_params and 'health' in output_params ) or ( output_params and 'diseased_zones' in output_params ) or not output_params:
            ...
            all_unhealthy_zones = db.session.query(func.count(DataPicture.id)).join(Data).join(Sensor).filter(DataPicture.results.like('%unhealthy%')).filter(DataPicture.ts >= ts_from).filter(DataPicture.ts <= ts_to).filter(DataPicture.ts.hour > 7).filter(DataPicture.ts.hour < 19)
            # Fixed for new results
