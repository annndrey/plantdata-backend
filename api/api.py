    ...
    def get(self):
        ...
        * number of unusual spikes of sensors data
        ...
        if ( output_params and 'health' in output_params ) or ( output_params and 'diseased_zones' in output_params ) or not output_params:
            ...
            grouped_zones = db.session.query(PictureZone.ts, func.count(PictureZone.id)).filter(PictureZone.results.like('%unhealthy%')).filter(PictureZone.ts >= grouped_ts_from).filter(PictureZone.ts <= ts_to).join(DataPicture).join(Data).join(Sensor)
