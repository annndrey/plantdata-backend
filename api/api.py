...
def custom_serializer(data, cameras=None):
    ...
    for d in data:
        ...
        for r in d.records:
            if r.probe.id not in [362, 356] and r.probe.label:
...
class DataPictureSchema(ma.ModelSchema):
    ...
    health = ma.Function(lambda obj: obj.health if 7 < obj.ts.hour < 19 else 0)
...
class NotificationsAPI(Resource):
    ...
    def patch(self, id):
        ...
        app.logger.debug(["PATCH", id, request.json, user, isread])
        notification = db.session.query(Notification).filter(Notification.id == id).filter(Notification.user == user).first()
        if notification:
