...
class DataPictureSchema(ma.ModelSchema):
    ...
    numwarnings = ma.Function(lambda obj: obj.numwarnings if 9 < obj.ts.hour < 22 else 0)
    results = ma.Function(lambda obj: obj.results if 9 < obj.ts.hour < 22 else "[]")
    ...
    def get(self):
        ...
        for c in outdata:
            ...
            outdict = {"ts": c[0].data.ts,
                       ...
                       "y": y,
                       "z": z,
                       "numwarnings":c[0].numwarnings if 9 < c[0].data.ts.hour < 22 else 0,
    ...
    def get(self):
        ...
        * number of unusual spikes of sensors data
        ...
        if ( output_params and 'health' in output_params ) or ( output_params and 'diseased_zones' in output_params ) or not output_params:
            ...
            all_unhealthy_zones = db.session.query(func.count(DataPicture.id)).join(Data).join(Sensor).filter(DataPicture.results.like('%unhealthy%')).filter(DataPicture.ts >= ts_from).filter(DataPicture.ts <= ts_to).filter(extract("hour", DataPicture.ts) > 9).filter(extract("hour", DataPicture.ts) < 22)
            # Fixed for new results
            grouped_zones = db.session.query(DataPicture.ts, func.count(DataPicture.id)).join(Data).join(Sensor).filter(DataPicture.results.like('%unhealthy%')).filter(DataPicture.ts >= grouped_ts_from).filter(DataPicture.ts <= ts_to).filter(extract("hour", DataPicture.ts) > 9).filter(extract("hour", DataPicture.ts) < 22)
